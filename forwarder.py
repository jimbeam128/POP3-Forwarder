...
for i in sorted(uidls.keys()):
    ...
    try:
        ...
        if any(
            ...
            print(
                f"[FILTER] Mail {i} gelöscht | Subject: {subject}"
            )
        ...
        reply_name, reply_addr = parseaddr(
            str(email_msg.get('Reply-To', ''))
        )

        return_path = email_msg.get(
            'Return-Path',
            ''
        )
        ...
        print(
            f"[FEHLER] Mail {i} | Subject: {safe_subject} | Error: {e}"
        )
